...
public class PaymentService {
    ...
    public PaymentResponse createPaymentUrl(Long bookingId, HttpServletRequest request) {
        ...
        if (booking.getStatus() != BookingStatus.PENDING) {
            throw new AppException(ErrorCode.INVALID_INPUT); // Hoặc tạo lỗi riêng cho trạng thái
        }

        long amount = (long) (booking.getTotalPrice() * 100); // VNPay yêu cầu nhân 100
    ...
    public void processVnpayReturn(HttpServletRequest request) {
        ...
        if (signValue.equals(vnp_SecureHash)) {
            if ("00".equals(request.getParameter("vnp_TransactionStatus"))) {
                ...
                throw new AppException(ErrorCode.INVALID_INPUT); // Giao dịch thất bại
            }
        } else {
            throw new AppException(ErrorCode.UNAUTHORIZED); // Chữ ký không hợp lệ
        }
    }
}
